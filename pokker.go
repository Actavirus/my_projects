...
func isHandOfStraights(hand []int, k int) bool {
	...
	// Шаг 4 : отсортируйте список и начните перемещаться по нему с 
	// карты с самым низким рейтингом. Мы можем использовать 
	// хэш-карту, сохраняя номера карт как ключи и вхождения как 
	// значения.
	sort.Ints(hand)
	i := 0
	n := len(hand)
	...
	for i < n {
		current := hand[i]
		for j := 0; j < k; j++ {
			// Шаг 5.1 : Проверьте, есть ли на карте текущая карта и 
			// следующие k-1 карты (в возрастающем рейтинге) count. 
			// Если какой-либо из них не существует, вернитесь false.
		...
		// следующей группы и определить, у какой из следующих карт 
		// countосталось больше нуля вхождений.
